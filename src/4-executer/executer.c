...
/*   Updated: 2024/02/20 14:22:45 by fschuber         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int execute_builtin(t_bin_tree_node *node, t_program_data *program_data)
{
	...
	// if (ft_strncmp(node->val[0]->value, "cd", 2) == 0) return execute_cd(node->val);
	// if (ft_strncmp(node->val[0]->value, "pwd", 3) == 0) return execute_pwd(node->val);
	// if (ft_strncmp(node->val[0]->value, "export", 6) == 0) return execute_export(node->val);
	// if (ft_strncmp(node->val[0]->value, "unset", 5) == 0) return execute_unset(node->val);
